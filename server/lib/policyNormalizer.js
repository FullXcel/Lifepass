...
function lawNameFromRecord(record = {}, fallback = '') {
  return cleanGenericLawTitle(
    record?.법령명한글 || record?.법령명 || record?.lawNm || record?.lsNm || extractXmlTag(String(record?.description || record?.rawText || ''), '법령명한글') || fallback,
  );
}
...
function inferLegalReferenceInfo(rawText = '', title = '', record = {}) {
  const lawName = lawNameFromRecord(record, title) || cleanGenericLawTitle(title) || '법령';
  const text = `${lawName}\n${rawText}`;
  const specific = LEGAL_ACT_ROLE_RULES.find((rule) => rule.pattern.test(text));
  if (specific) {
    return {
      related_policy_domains: specific.domains,
      legal_basis_summary: `${lawName}은 ${specific.target}의 판단 근거입니다. 직접 지급되는 혜택으로 계산하지 않고 자격·지원 기준 설명과 원문 확인용으로 분리합니다.`,
      legal_basis_role: specific.role,
      user_value: specific.userValue,
    };
  }
  ...
  return {
    ...
    legal_basis_summary: `${lawName}은 ${scope}와 연결되는 상위 근거입니다. 직접 지급되는 혜택으로 계산하지 않고 정책 판정 설명과 원문 확인용으로 분리합니다.`,
...
function extractPublicUrl(record = {}) {
  ...
  if (lawUrl) return { url: lawUrl, status: 'ok', reason: '법령명으로 국가법령정보센터 공개 원문 링크를 만들었습니다.' };
...
export function normalizePolicyRecord(record = {}, source = {}, context = {}) {
  ...
  const pickedTitle = pick(record, TITLE_KEYS) || lawNameFromRecord(record) || rawText.split(/\r?\n/).find((line) => !/^LawSearch:/i.test(line)) || '수집 정책';
  ...
  const legalInfo = domain === '법령근거' ? inferLegalReferenceInfo(rawText, title, record) : null;
