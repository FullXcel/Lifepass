...
- 외부 API는 HTTP 상태가 성공이어도 내부 응답 코드가 오류일 수 있습니다. 예를 들어 복지로 API는 인증키, 필수 파라미터, endpoint가 맞지 않으면 INVALID_REQUEST_PARAMETER_ERROR 같은 응답을 반환할 수 있으므로 수집 로그와 검수 대기 데이터를 함께 확인해야 합니다.
